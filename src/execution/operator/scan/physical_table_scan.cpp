							...
							case TableFilterType::CONSTANT_COMPARISON: {
								...
								if (constant_comp_filter.constant.type().id() == LogicalTypeId::DECIMAL) {
									...
									case PhysicalType::INT16: {
										...
										TryCastFromDecimal::Operation(value16, discount, nullptr, 15, 2);
										break;
									}
									case PhysicalType::INT32: {
										int32_t value32 = constant_comp_filter.constant.GetValueUnsafe<int32_t>();
										TryCastFromDecimal::Operation(value32, discount, nullptr, 15, 2);
										break;
									}
									case PhysicalType::INT64: {
										int64_t value64 = constant_comp_filter.constant.GetValueUnsafe<int64_t>();
										TryCastFromDecimal::Operation(value64, discount, nullptr, 15, 2);
										break;
									}
									default:
...
void PhysicalTableScan::IndexRead(vector<row_t> *row_ids, uint64_t begin, uint64_t end, nbub::Nbub *bitmap_shipdate,
	...
	uint64_t local_local_revenue = 0;
	for (size_t i = 0; i < local_chunks.size(); i++) {
		...
		for (size_t i = 0; i < chunk->size(); i++) {
			auto lentry = vec1_data[i];
			auto rentry = vec2_data[i];
