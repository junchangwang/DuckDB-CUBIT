...
po::variables_map get_options(const int argc, const char *argv[]) {
    ...
    desc.add_options()
            ...
            ("approach,a", po::value<std::string>()->default_value(std::string("ub")), "naive, ucb, ub, nbub-lk, nbub-lf")
            ("mode,m", po::value<std::string>(), "build / query / test")
            ("encoding-scheme,e", po::value<std::string>()->default_value("EE"), "EE / RE / AE / IE")
            ...
            ("range-algorithm", po::value<std::string>()->default_value(std::string("pq")), "pq, naive, uncompress");          
...
void init_config(Table_config *config, po::variables_map options) {
    ...
    config->approach = options["approach"].as<std::string>();
    ...
    config->range_algo = options["range-algorithm"].as<std::string>();
    ...
    if (config->segmented_btv) {
        ...
        std::cout << "=== Using Segmented Btv. Row per seg: " << config->rows_per_seg << std::endl;
    }
}
