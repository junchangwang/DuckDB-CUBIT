...
NbubLF::NbubLF(Table_config *config) : nbub::Nbub(config)
{
...
int NbubLF::help_insert_trans(TransDesc *tail, TransDesc *trans)
{ 
...
int NbubLF::merge_bitmap(int tid, uint32_t val, TransDesc *trans,
        nbub::Bitmap *bitmap_old, nbub::Bitmap *bitmap_new, std::map<uint64_t, RUB> *rubs)
{
    /* Prepare trans_merge */
