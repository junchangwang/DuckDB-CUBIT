...
void TableX::appendRow(int val, Table_config *config)
{
    switch (config->encoding)
    {
        ...
        case AE:
             ...
             break;
        default:
             std::cout << "Unknown encoding scheme: " << config->encoding << std::endl;
             assert(0);
    }
...
void TableX::check() {
    std::cout << "Build index done with " << number_of_rows << " rows" << std::endl;
    for (int i = 0; i < cardinality; i++) {
        std::cout << "cnt of bitmap " << i << " is " << bitmaps[i]->do_cnt() << std::endl;
    }
}
